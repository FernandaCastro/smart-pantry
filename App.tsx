        ...
        <div className="w-full max-w-2xl bg-[var(--sp-slate-700)] rounded-[2.5rem] p-8 shadow-2xl border border-[var(--sp-slate-700)]">
          ...
          <div className="bg-[var(--sp-slate-900)] rounded-2xl p-6 mb-6 relative group">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={handleCopySql} className="p-2 bg-[var(--sp-slate-700)] hover:bg-[var(--sp-slate-700)] text-[var(--sp-slate-300)] rounded-xl transition-colors flex items-center gap-2 text-xs font-bold">
          ...
          <div className="space-y-4">
            ...
            <div className="pt-4 flex gap-3">
              <a href={SUPABASE_URL.replace('https://', 'https://app.supabase.com/project/')} target="_blank" rel="noopener noreferrer" className="flex-1 bg-[var(--sp-violet-600)] text-[var(--sp-white)] py-4 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-[var(--sp-violet-700)] transition-all">
          ...
          <div className="space-y-6">
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleAuth(); }} className="space-y-4">
              {isRegistering && (
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="text" required className="w-full p-4 pl-12 bg-[var(--sp-gray-50)] rounded-2xl border-2 border-transparent focus:border-[var(--sp-violet-500)] outline-none transition-all" placeholder="Seu Nome Completo" value={authName} onChange={e => setAuthName(e.target.value)} />
                  </div>
                </div>
              )}
              ...
              <div>
                ...
                <div className="relative">
                  <UserIcon className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--sp-gray-400)]" size={18} />
                  <input type="email" required className="w-full p-4 pl-12 bg-[var(--sp-gray-50)] rounded-2xl border-2 border-transparent focus:border-[var(--sp-violet-500)] outline-none transition-all" placeholder="seu@email.com" value={authEmail} onChange={e => setAuthEmail(e.target.value)} />
                </div>
              </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-[var(--sp-violet-100)] via-[var(--sp-indigo-50)] to-[var(--sp-fuchsia-100)] lg:p-6 relative overflow-hidden">
      ...
      <aside className="hidden lg:flex w-72 border-r border-[color:color-mix(in_srgb,var(--sp-white)_70%,transparent)] bg-gradient-to-b from-[color:color-mix(in_srgb,var(--sp-violet-100)_80%,transparent)] via-[color:color-mix(in_srgb,var(--sp-indigo-50)_70%,transparent)] to-[color:color-mix(in_srgb,var(--sp-white)_80%,transparent)] p-6 flex-col gap-8">
        ...
        <div className="mt-auto space-y-3">
          <button onClick={() => setCurrentView('pantry')} className="w-full text-left bg-[var(--sp-white)] rounded-2xl p-4 border border-[var(--sp-violet-100)] hover:border-[var(--sp-violet-200)] transition-all cursor-pointer active:scale-[0.99]">
          ...
          <button onClick={() => setCurrentView('shopping')} className="w-full text-left bg-[var(--sp-white)] rounded-2xl p-4 border border-[var(--sp-indigo-100)] hover:border-[var(--sp-indigo-200)] transition-all cursor-pointer active:scale-[0.99]">
            <div className="flex items-center justify-between gap-2">
      ...
      <main className="flex-1 p-4 lg:p-6 overflow-y-auto">
          ...
          <div className="space-y-6">
            <div className="bg-gradient-to-br from-[var(--sp-violet-500)] to-[var(--sp-violet-700)] rounded-[2rem] p-6 text-[var(--sp-white)] relative overflow-hidden shadow-xl">
               <div className="relative z-10">
                 ...
                 <button onClick={handleFetchAiSuggestions} disabled={isLoading} className="bg-[color:color-mix(in_srgb,var(--sp-white)_20%,transparent)] hover:bg-[color:color-mix(in_srgb,var(--sp-white)_30%,transparent)] backdrop-blur-md border border-[color:color-mix(in_srgb,var(--sp-white)_30%,transparent)] px-5 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition-all active:scale-95">
            ...
            <div className="space-y-6 lg:space-y-0 lg:grid lg:grid-cols-3 lg:gap-6 lg:items-start">
              <div className="grid grid-cols-2 gap-4 lg:hidden">
                <button onClick={() => setCurrentView('pantry')} className="text-left bg-[var(--sp-violet-50)] p-4 rounded-3xl border border-[var(--sp-violet-100)] cursor-pointer active:scale-[0.99] transition-all">
                  <div className="flex items-center justify-between gap-2">
                ...
                <button onClick={() => setCurrentView('shopping')} className="text-left bg-[var(--sp-indigo-50)] p-4 rounded-3xl border border-[var(--sp-indigo-100)] cursor-pointer active:scale-[0.99] transition-all">
                  <div className="flex items-center justify-between gap-2">
          ...
          <div className="space-y-4 lg:space-y-6">
            <div className="flex items-center gap-3 lg:max-w-2xl">
              <div className="relative flex-1">
                ...
                <input type="text" placeholder={t('searchPlaceholder')} className="w-full pl-12 pr-4 py-4 bg-[var(--sp-gray-50)] rounded-2xl border-2 border-transparent focus:border-[var(--sp-violet-500)] outline-none" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
              </div>
              <button onClick={() => setIsModalOpen(true)} className="w-14 h-14 bg-[var(--sp-violet-100)] text-[var(--sp-violet-600)] rounded-2xl shadow-lg border border-[var(--sp-violet-200)] flex items-center justify-center active:scale-90 transition-all"><Plus size={24} /></button>
            </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-1 flex items-center justify-between">
                        <div className="flex items-center gap-1">
                          <button onClick={() => updateQuantity(item.id, -1)} className="p-2 text-[var(--sp-gray-400)]"><Minus size={18} /></button>
                          <span className="w-8 text-center font-bold">{item.currentQuantity}</span>
                          <button onClick={() => updateQuantity(item.id, 1)} className="p-2 text-[var(--sp-gray-400)]"><Plus size={18} /></button>
                        </div>
                        <div className="flex items-center gap-1">
                          <button onClick={() => handleEditClick(item)} className="p-1.5 text-[var(--sp-gray-300)] hover:text-[var(--sp-violet-500)]"><Pencil size={16} /></button>
                          <button onClick={() => handleDeleteProduct(item.id)} className="p-1.5 text-[var(--sp-gray-300)] hover:text-[var(--sp-red-400)]"><Trash2 size={16} /></button>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div className="space-y-4 pb-32 lg:pb-8">
                        ...
                        {group.items.map(item => (
                          <div key={item.id} className={`bg-[var(--sp-white)] border-2 p-4 rounded-3xl flex flex-col gap-4 ${selectedShopItems[item.id] ? 'border-[var(--sp-violet-500)] bg-[color:color-mix(in_srgb,var(--sp-violet-50)_30%,transparent)]' : 'border-[var(--sp-gray-50)]'}`}>
                            <div className="flex items-center gap-4">
                              <button onClick={() => setSelectedShopItems(prev => ({...prev, [item.id]: !prev[item.id]}))} className={selectedShopItems[item.id] ? 'text-[var(--sp-violet-600)]' : 'text-[var(--sp-gray-300)]'}>
                            ...
                            {selectedShopItems[item.id] && (
                              <div className="flex items-center justify-between p-3 bg-[var(--sp-white)] rounded-2xl border border-[var(--sp-violet-100)]">
                                ...
                                <div className="flex items-center gap-3">
                                  <button onClick={() => setShopQuantities(prev => ({...prev, [item.id]: Math.max(0, (shopQuantities[item.id] || item.neededQuantity) - 1)}))} className="w-8 h-8 rounded-full bg-[var(--sp-violet-100)] text-[var(--sp-violet-600)] flex items-center justify-center"><Minus size={14}/></button>
                                  <span className="font-black">{shopQuantities[item.id] !== undefined ? shopQuantities[item.id] : item.neededQuantity}</span>
                                  <button onClick={() => setShopQuantities(prev => ({...prev, [item.id]: (shopQuantities[item.id] || item.neededQuantity) + 1}))} className="w-8 h-8 rounded-full bg-[var(--sp-violet-100)] text-[var(--sp-violet-600)] flex items-center justify-center"><Plus size={14}/></button>
                                </div>
                              </div>
                            )}
                          </div>
            ...
            {Object.values(selectedShopItems).some(v => v) && (
              <div className="fixed bottom-24 left-4 right-4 z-[60] lg:static lg:bottom-auto lg:left-auto lg:right-auto lg:mt-2 lg:max-w-sm lg:ml-auto">
                <button onClick={handleFinishPurchase} disabled={isLoading} className="w-full bg-[var(--sp-violet-600)] text-[var(--sp-white)] py-4 rounded-2xl font-bold shadow-lg flex items-center justify-center gap-3">
        ...
        {currentView === 'settings' && (
          <div className="space-y-8">
            <div className="flex items-center gap-4 p-5 bg-[var(--sp-gray-50)] rounded-[2rem]">
              ...
              <button onClick={handleLogout} className="p-3 text-[var(--sp-red-400)]"><LogOut size={20} /></button>
            </div>
        ...
        {currentView === 'ai' && (
          <div className="space-y-6">
             ...
             <button onClick={() => setCurrentView('dashboard')} className="w-full py-4 border-2 border-[var(--sp-violet-500)] text-[var(--sp-violet-500)] font-bold rounded-2xl">{t('back')}</button>
          </div>
        )}
      </main>
